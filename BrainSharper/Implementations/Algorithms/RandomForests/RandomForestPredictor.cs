...
namespace BrainSharper.Implementations.Algorithms.RandomForests
{
    public class RandomForestPredictor<TPredictionVal> : IPredictor<TPredictionVal>
    {
        ...
        public RandomForestPredictor(IPredictor<TPredictionVal> decisionTreePredictor,
            bool votingWeightedByOobError = false)
        {
            this.decisionTreePredictor = decisionTreePredictor;
            useVotingWeightedByOob = votingWeightedByOobError;
        }

        public IList<TPredictionVal> Predict(IDataFrame queryDataFrame, IPredictionModel model,
            string dependentFeatureName)
        {
            ...
            {
                for (var rowIdx = 0; rowIdx < queryDataFrame.RowCount; rowIdx++)
                {
            ...
            var results = predictionVotes.Select(
                rowVotes =>
                    rowVotes.Value.OrderByDescending(weightedPredictions => weightedPredictions.Value).First().Key)
                .ToList();
